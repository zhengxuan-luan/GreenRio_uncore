...
CACHE_SRC := $(shell find svvfile -type f -name '*.[v]')
...
verify_cache: 
	$(verilator) $(VERIFLAGS) -top-module cache_utop +define+RUBY --Wno-LATCH --Wno-COMBDLY $(CACHE_SRC)  $(DV_ROOT)/cache_ut/cache_sim.cpp
	-build/Vcache_utop
